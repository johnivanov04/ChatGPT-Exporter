      ...
      },
      {
        text: (
          <>
            ...
            <code className="text-[11px] bg-slate-800 rounded px-1 py-0.5">
              conversations.json
            </code>{" "}
            inside it.
          </>
...
export function HowToExport({
  ...
  return (
    <div className="mt-6 rounded-2xl border border-slate-700/80 bg-slate-900/60 backdrop-blur-sm overflow-hidden">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="w-full flex items-center justify-between gap-3 px-5 py-3.5 text-left hover:bg-slate-900/80 transition focus-ring"
      >
        ...
        <div className="border-t border-slate-800 animate-fade-up">
          ...
          <div className="px-5 pb-5 pt-3">
            ...
            <ol className="space-y-2.5">
              {guide.steps.map((step, i) => (
                <li key={i} className="flex gap-3 text-sm text-slate-200">
                  <span className="shrink-0 inline-flex h-5 w-5 items-center justify-center rounded-full bg-slate-800 text-slate-200 text-[11px] font-semibold mt-0.5">
                    {i + 1}
                  </span>
            ...
            {guide.tips && guide.tips.length > 0 && (
              <div className="mt-4 rounded-lg bg-amber-500/10 border border-amber-500/30 px-3.5 py-2.5">
                <p className="text-[11px] uppercase tracking-wider font-semibold text-amber-300 mb-1">
                  Tips
                </p>
                <ul className="space-y-1">
                  {guide.tips.map((tip, i) => (
                    <li key={i} className="text-[13px] text-slate-200 leading-relaxed">
                      &middot; {tip}
                    </li>
                  ))}
                </ul>
...
function ExtLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className="inline-flex items-center gap-0.5 text-amber-300 hover:text-amber-100 underline underline-offset-2"
    >
      {children}
